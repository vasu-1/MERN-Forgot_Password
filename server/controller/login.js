...
router.post('/api/v1/signup', async (req, res) => {
  ...
  try {
    ...
    if (userRegister) {
      ...
      return res.status(201).json({ message: "Pin Sent Successfully, Kindly Check Your Email!" });
    } else {
...
router.post('/api/v1/verify', async (req, res) => {
    ...
    try {
      ...
      if (emailExist) {
        ...
        return res.status(201).json({ message: "You are verified Successfully!" , jwttokenloginuser: emailExist.token});
      } else {
...
router.post('/api/v1/signin', async (req, res) => {
  ...
  try {
    ...
    var usernameExist = await User.findOne({ email: verifytoken.email, token: token });
    // const usernameExist = await User.findOne({ username: username });
    if (usernameExist) {
      const isMatch = await bcrypt.compare(password, usernameExist.password);

      if (isMatch) {
        return res.status(201).json({ message: "You are logged in!", jwttokenloginuser: usernameExist.token, userId: usernameExist._id });
      } else {
